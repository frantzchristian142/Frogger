...
namespace ZeEngine
{
    public class Animated2d : Basic2d
    {
        ...
        public virtual void OutsideBounds()
        {
            if (isAlive)
            {
                if(bounds.Width < 0 || bounds.X > GameGlobals.LEVEL_WIDTH * GameGlobals.TILE_OFFSET || bounds.Height > GameGlobals.LEVEL_HEIGHT * GameGlobals.TILE_OFFSET || bounds.Y < 0)
                {
                    isAlive = false;
                }
            }
        ...
        public override void Draw()
        {
            ...
            if(frameAnimations && frameAnimationList[currentAnimation].Frames > 0)
            {
                ...
                frameAnimationList[currentAnimation].Draw(myModel, dims, frameSize, updateOffset, pos, rot, color, new SpriteEffects());
